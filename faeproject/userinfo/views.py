...
from .models import *
from forminfo.models import FormInfoPlan , FormInfoEvent , FormInfoWork , CommentPlan , CommentEvent , CommentWork
import logging
...
def wel_1(request):
    ...
    plan_comment = UserInfo.objects.filter(forminfoplan__nums__gt=0).annotate(num_plancomment=Count('forminfoplan')).filter(num_plancomment__gt=0)
    event_comment = UserInfo.objects.filter(forminfoevent__nums__gt=0).annotate(num_eventcomment=Count('forminfoevent')).filter(num_eventcomment__gt=0)
    work_comment = UserInfo.objects.filter(forminfowork__nums__gt=0).annotate(num_workcomment=Count('forminfowork')).filter(num_workcomment__gt=0)
    ...
    "plan_comment":plan_comment,"event_comment":event_comment,"work_comment":work_comment}
